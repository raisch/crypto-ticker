...
const {_, stringify} = require('./util')
...
class Ticker extends EventEmitter {
  ...
  constructor (opts = {}) {
    ...
    this.on('DisplayData', this.onDisplayData)
  ...
  convert (data) {
    if (!_.isFunction(this._converter)) {
      throw new Error('requires a converter')
    }
  ...
  onConvertData (data) {
    ...
    this.emit('DisplayData', result)
  }

  /**
   * Displays information from data using this._writer.
